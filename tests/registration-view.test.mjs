...
const render = (view, data) => renderToStaticMarkup(createElement(view, { data, onNavigate() {}, onNew() {}, onEdit() {}, onStatus() {}, onSetPassword() {} })).replace(/<[^>]*>/g," ").replace(/\s+/g," ");
...
test("dashboard distinguishes work date from submission time in the organization timezone", () => {
  ...
  const text = render(views.Overview, data);
  ...
  assert.match(render(views.Overview,data), /04\/08\/2026,? 10:00/);
});
...
test("invalid dates are disclosed and do not appear as a zero-delay average", () => {
  ...
  for (const view of [views.Overview, views.TeamView]) {

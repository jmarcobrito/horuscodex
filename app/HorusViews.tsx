...
export function Overview({ data, onNavigate }: { data: DashboardData; onNavigate: (section: Section) => void }) {
  ...
  return <>
    ...
    <section className="dashboard-grid"><div className="panel discipline-panel"><PanelHeading eyebrow="DISCIPLINA DE PREENCHIMENTO" title="Acompanhamento das pessoas" action="Ver todas as pessoas" onAction={() => onNavigate("team")} />{data.contractors.filter((person) => person.status === "ACTIVE").length ? <div className="table-scroll"><table><thead><tr><th>Colaborador</th><th>Último lançamento</th><th>Atraso médio</th><th>Retroativos</th><th>Dias com lançamento</th><th>Horas em relação à carga mensal</th></tr></thead><tbody>{data.contractors.filter((person) => person.status === "ACTIVE").map((person) => <tr key={person.id}><td><div className="person-cell"><span className="mini-avatar violet">{person.initials}</span><div><strong>{person.name}</strong><small>{person.email}</small></div></div></td><td>{person.lastEntryDate ? formatDate(person.lastEntryDate) : "Sem lançamentos"}</td><td>{person.averageDelayDays} dia(s)</td><td>{person.retroactiveEntries}</td><td>{display.daysByPerson[person.id] ?? 0}</td><td>{display.fullMonth ? <><div className="fill-cell"><div className="mini-progress"><span style={{ width: person.fillPercentage + "%" }} /></div><b>{person.fillPercentage}%</b></div>{(person.estimatedRequiredMonths ?? 0) > 0 && <small>Inclui estimativa da carga</small>}</> : <small>Consulte um mês completo</small>}</td></tr>)}</tbody></table></div> : <Empty text="Nenhum colaborador ativo encontrado." />}</div>
...
function MonthlyContext({ context, estimatedMonths }: { context: ReturnType<typeof dashboardDisplay>["monthlyContext"]; estimatedMonths: number }) {
  return <section className="monthly-context" aria-label="Contexto dos meses consultados">
    ...
    {context === null ? <p role="status">Contexto mensal indisponível</p> : <>
      <div className="timesheet-summary">
        <div><span>Abonos dos meses</span><strong>{formatMinutes(context.creditedMinutes)}</strong></div>
        <div><span>Carga dos meses</span><strong>{formatMinutes(context.requiredMinutes)}</strong>{estimatedMonths > 0 && <small>Inclui estimativa para meses sem registro mensal</small>}</div>
        <div className="projected"><span>Projeção dos meses</span><strong>{formatMinutes(context.projectedBalanceMinutes, true)}</strong><small>Totais dos registros mensais menos a carga dos meses</small></div>
      </div>
    </>}
  </section>;
}

...
export type Section = "overview" | "entries" | "balance" | "requests" | "team" | "reports";
...
export function EntriesView({ role, data, onNew, onEdit, onHistory }: { role: Role; data: DashboardData; onNew: () => void; onEdit: (entry: DashboardEntry) => void; onHistory: (entry: DashboardEntry) => void }) {
  return <><section className="page-heading"><div><span className="eyebrow">PERÍODO SELECIONADO</span><h1>{role === "rh" ? "Lançamentos da equipe" : "Meu mês"}</h1><p>{monthLabel(data.period)}</p></div>{role === "pj" && <button className="primary-button" onClick={onNew}>+ Registrar horas</button>}</section>
    ...
    <section className="panel entries-panel">{data.entries.length ? <div className="entry-list">{data.entries.map((entry) => <article key={entry.id} className="entry-row"><div className="date-tile"><strong>{entry.workDate.slice(8, 10)}</strong><span>{entry.workDate.slice(5, 7)}</span></div><div className="entry-day"><strong>{role === "rh" ? entry.contractorName : entryDay(entry)}</strong><span>{entry.notes || entryDay(entry)}</span></div><div className="time-block"><span>Entrada</span><strong>{entry.startTime}</strong></div><div className="time-separator">→</div><div className="time-block"><span>Saída</span><strong>{entry.endTime}</strong></div><div className="time-block break-block"><span>Intervalo</span><strong>{formatMinutes(entry.breakMinutes)}</strong></div><div className="entry-total"><span>Total</span><strong>{formatMinutes(entry.calculatedMinutes)}</strong></div><span className={"status-pill " + statusTone(entry.nonBusinessDayStatus)}>{statusLabel(entry.nonBusinessDayStatus)}</span><div className="row-actions"><button onClick={() => onEdit(entry)}>Editar</button><button onClick={() => onHistory(entry)}>Histórico</button></div></article>)}</div> : <Empty text="Nenhum lançamento no período selecionado." />}</section></>;
}
...
export function TeamView({ data, onNew, onStatus, onDelete, onTimesheet, onSetPassword }: { data: DashboardData; onNew: () => void; onStatus: (id: string, next: "ACTIVE" | "INACTIVE") => void; onDelete: (id: string, name: string) => void; onTimesheet: (id: string, action: "CLOSE" | "REOPEN") => void; onSetPassword: (id: string, name: string) => void }) {
  return <><section className="page-heading"><div><span className="eyebrow">{data.metrics.activeContractors} PRESTADORES ATIVOS</span><h1>Equipe</h1><p>Cadastros, acessos, disciplina de preenchimento e fechamento por competência.</p></div><button className="primary-button" onClick={onNew}>+ Novo prestador</button></section><section className="team-grid">{data.contractors.length ? data.contractors.map((person) => <article className={"team-card " + (person.status === "INACTIVE" ? "inactive" : "")} key={person.id}><div className="team-avatar violet">{person.initials}</div><div className="team-person"><h3>{person.name}</h3><span>{person.email}</span></div><span className={"status-pill " + (person.status === "ACTIVE" ? "success" : "neutral")}>{person.status === "ACTIVE" ? "Ativo" : "Inativo"}</span><div className="team-stats"><p><span>Preenchimento</span><strong>{person.fillPercentage}%</strong></p><p><span>Atraso médio</span><strong>{person.averageDelayDays}d</strong></p></div><div className="team-progress"><span style={{ width: person.fillPercentage + "%" }} /></div><div className="team-actions"><button onClick={() => onSetPassword(person.id, person.name)}>Definir senha</button><button onClick={() => onStatus(person.id, person.status === "ACTIVE" ? "INACTIVE" : "ACTIVE")}>{person.status === "ACTIVE" ? "Inativar" : "Reativar"}</button>{data.period.year && data.period.month && person.status === "ACTIVE" && (person.timesheetStatus === "CLOSED" ? <button className="approve" onClick={() => onTimesheet(person.id, "REOPEN")}>Reabrir competência</button> : <button className="approve" onClick={() => onTimesheet(person.id, "CLOSE")}>Fechar competência</button>)}<button className="delete-action" onClick={() => onDelete(person.id, person.name)}>Excluir</button></div><small className="competence-state">Competência: {statusLabel(person.timesheetStatus)}</small></article>) : <Empty text="Nenhum prestador cadastrado." />}</section></>;
}

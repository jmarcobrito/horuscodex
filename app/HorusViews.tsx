...
export function ReportsView({ data, onPolicy }: { data: DashboardData; onPolicy: () => void }) {
  ...
  return <><section className="page-heading"><div><span className="eyebrow">RELATÓRIOS E CONTROLE</span><h1>Relatórios</h1><p>Exportações respeitam o período selecionado nesta tela.</p></div><button className="secondary-button" onClick={onPolicy}>Configurar políticas</button></section><section className="report-grid"><ReportCard title="Lançamentos" text="Horas trabalhadas, consideradas, atraso e observações." href={`/api/reports/export?type=entries&${base}`} /><ReportCard title="Banco de horas" text="Lotes, origem, prazo, reservas e saldos remanescentes." href={`/api/reports/export?type=balances&${base}`} /><ReportCard title="Auditoria" text="Ações, responsáveis e justificativas registradas." href={`/api/reports/export?type=audit&${base}`} /></section></>;
}

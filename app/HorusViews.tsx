...
export type Section = "overview" | "entries" | "balance" | "approvals" | "closing" | "team" | "reports" | "admin";
...
export function Overview({ data, loading, onNavigate, onPeriod }: { data: DashboardData; loading: boolean; onNavigate: (section: Section) => void; onPeriod: (query: string) => void }) {
  ...
  return <>
    <section className="page-heading overview-heading"><div><span className="eyebrow">VISÃO CONSOLIDADA</span><h1>Painel</h1><p>{monthLabel(data.period)} · dados reais da organização</p></div>{pending > 0 && <button className="secondary-button" onClick={() => onNavigate("approvals")}>{pending} pendência(s)</button>}</section>

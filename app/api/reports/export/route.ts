...
export async function GET(request: Request) {
  try {
    ...
    if (type === "balances") {
      const result = await admin.from("hour_balance_lots")
      ...
      const result = await admin.from("audit_logs").select("created_at,action,entity_type,entity_id,reason,users!audit_logs_user_id_fkey(name,email)")
        .eq("organization_id", actor.organizationId).gte("created_at", from + "T00:00:00").lte("created_at", to + "T23:59:59.999").order("created_at");
      if (result.error) throw result.error;
      rows = [["Data", "Responsável", "E-mail", "Ação", "Entidade", "ID", "Justificativa"],
        ...(result.data ?? []).map((row: Record<string, unknown>) => { const user = row.users as { name?: string; email?: string } | null; return [row.created_at, user?.name, user?.email, row.action, row.entity_type, row.entity_id, row.reason]; })];
    } else {
      const result = await admin.from("time_entries")

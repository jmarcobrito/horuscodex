...
export function HorusApp({ user, role, organizationName, initialDashboard }: { user: User; role: Role; organizationName: string; initialDashboard: DashboardData }) {
  ...
  const [contractorPasswordForm, setContractorPasswordForm] = useState({ id: "", name: "", password: "" });
  ...
  async function submitContractorPassword(event: FormEvent<HTMLFormElement>) {
    ...
    const success = await mutate("/api/team", "PATCH", { id: contractorPasswordForm.id, action: "SET_PASSWORD", password: contractorPasswordForm.password }, "Senha do prestador atualizada.");
    if (success) setContractorPasswordForm({ id: "", name: "", password: "" });
  }
  ...
  return <div className="app-shell">
    ...
    {modal === "contractorPassword" && <Modal title="Definir senha" eyebrow="ACESSO DO PRESTADOR" description={`Crie uma nova senha para ${contractorPasswordForm.name}.`} onClose={() => setModal(null)}><form onSubmit={submitContractorPassword}><label className="field full-field">Nova senha<input type="password" autoComplete="new-password" value={contractorPasswordForm.password} onChange={(event) => setContractorPasswordForm({ ...contractorPasswordForm, password: event.target.value })} minLength={8} maxLength={72} required autoFocus /></label><div className="audit-note"><span>◈</span><p><strong>Compartilhamento seguro</strong>A senha não será enviada por e-mail nem registrada na auditoria.</p></div><ModalActions loading={loading} onCancel={() => setModal(null)} label="Salvar nova senha" /></form></Modal>}

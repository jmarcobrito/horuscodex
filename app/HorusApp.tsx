...
export function HorusApp({ user, role, organizationName, initialDashboard }: { user: User; role: Role; organizationName: string; initialDashboard: DashboardData }) {
  ...
  async function mutate(path: string, method: "POST" | "PATCH", body: unknown, success: string, closeModal = true) {
    ...
    try {
      ...
      await fetchDashboard(); if (closeModal) setModal(null); showNotice(success); return true;
  ...
  async function submitContractor(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const success = await mutate("/api/team", "POST", contractorForm, "Prestador cadastrado. Ele poderá acessar com este e-mail."); if (success) setContractorForm({ name: "", email: "" });
  }
  ...
  return <div className="app-shell">
    ...
    {modal === "contractor" && <Modal title="Novo prestador" eyebrow="CADASTRO PJ" description="O e-mail será usado para vincular a identidade no primeiro acesso." onClose={() => setModal(null)}><form onSubmit={submitContractor}><label className="field full-field">Nome completo<input value={contractorForm.name} onChange={(event) => setContractorForm({ ...contractorForm, name: event.target.value })} maxLength={200} required /></label><label className="field full-field">E-mail<input type="email" value={contractorForm.email} onChange={(event) => setContractorForm({ ...contractorForm, email: event.target.value })} maxLength={320} required /></label><ModalActions loading={loading} onCancel={() => setModal(null)} label="Cadastrar prestador" /></form></Modal>}

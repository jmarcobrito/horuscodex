...
export type HistoryVersion = { id: string; version_number: number; previous_data: Record<string, unknown>; new_data: Record<string, unknown>; changed_by: string; change_reason: string | null; changed_at: string };
...
const fields = [["Entrada", "start_time"], ["Saída", "end_time"], ["Intervalo", "break_minutes"], ["Observação", "notes"], ["Horas calculadas", "calculated_minutes"]] as const;
export function historyFields(version: HistoryVersion) {
  const display = (data: Record<string, unknown>, key: string) => {
    ...
    if (key === "calculated_minutes") return Number.isFinite(Number(value)) ? formatMinutes(Number(value)) : "Não informado";
...
export function EntryHistory({ state, names, onRetry }: { state: HistoryState; names: ReadonlyMap<string, string>; onRetry: () => void }) {
  ...
  return <div className="history-list">{state.versions.map(version => {
    ...
    return <article key={version.id}>
      ...
      <p><time>{Number.isNaN(date.getTime()) ? "Data não disponível" : new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(date)}</time> · {names.get(version.changed_by) ?? "Responsável não identificado"}</p>

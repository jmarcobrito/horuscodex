...
import { buildApprovalInbox } from "./approval-inbox";
import { getSupabaseAdmin } from "./supabase";
import { normalizeTimesheetPreview } from "./timesheet-preview";
...
type TimesheetRow = { contractor_id: string; year: number; month: number; required_minutes: number; credited_minutes: number; status: "OPEN" | "CLOSED" | "REOPENED" };
...
type AuditRow = { id: string; user_id: string | null; actor_name: string | null; action: string; entity_type: string; entity_id: string; reason: string | null; created_at: string };
...
export async function getDashboardData(actor: HorusActor, input: PeriodInput = {}): Promise<DashboardData> {
  ...
  let timesheetsQuery = admin.from("monthly_timesheets").select("contractor_id,year,month,required_minutes,credited_minutes,status").eq("organization_id", actor.organizationId);
  ...
  let requestsQuery = admin.from("leave_requests").select("id,contractor_id,start_date,end_date,requested_minutes,reserved_minutes,status,reason,requested_at,decision_notes").eq("organization_id", actor.organizationId).lte("start_date", period.to).gte("end_date", period.from).order("requested_at", { ascending: false }).limit(100);
  ...
  let auditsQuery = admin.from("audit_logs").select("id,user_id,actor_name,action,entity_type,entity_id,reason,created_at").eq("organization_id", actor.organizationId).order("created_at", { ascending: false }).limit(80);
  if (actor.role === "PJ") {
  ...
  const closingPreviews = period.year && period.month
    ? await Promise.all(activeUsers.map(async (user) => {
        ...
        return normalizeTimesheetPreview(result.data);
      }))
    : [];
  const previewsByContractor = new Map(closingPreviews.map((preview) => [preview.contractorId, preview]));
  ...
  const dashboardAudits: DashboardAudit[] = audits.map((row) => ({ id: row.id, userName: row.actor_name ?? (row.user_id ? names.get(row.user_id) : null) ?? (row.user_id === actor.id ? actor.name : "Usuário"), action: row.action, entityType: row.entity_type, entityId: row.entity_id, reason: row.reason ?? "", createdAt: row.created_at }));
  ...
  const workedMinutes = closingPreviews.length ? sum(closingPreviews, (preview) => preview.workedMinutes) : sum(activeEntries, (entry) => entry.calculated_minutes); const creditedMinutes = closingPreviews.length ? sum(closingPreviews, (preview) => preview.creditedMinutes) : sum(activeTimesheets, (row) => row.credited_minutes); const consideredMinutes = closingPreviews.length ? sum(closingPreviews, (preview) => preview.consideredMinutes) : sum(activeEntries, (entry) => entry.eligible_minutes) + creditedMinutes;
  const requiredMinutes = closingPreviews.length ? sum(closingPreviews, (preview) => preview.requiredMinutes) : activeTimesheets.length ? sum(activeTimesheets, (row) => row.required_minutes) : requiredPerMonth * activeUsers.length * keys.size; const statuses = new Set(activeTimesheets.map((row) => row.status));
  ...
  return {
    ...
    timesheet: { workedMinutes, creditedMinutes, consideredMinutes, requiredMinutes, projectedBalanceMinutes: consideredMinutes - requiredMinutes, status: statuses.size === 1 ? [...statuses][0] : statuses.size > 1 ? "MIXED" : "NOT_STARTED" },
